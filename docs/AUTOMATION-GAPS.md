...
| Abandoned jobs still hang on GitHub | **Partly closed.** GitHub has no per-job cancel — confirmed against its REST API, where every workflow-job endpoint is a read — so a job this node gives up on stays queued until GitHub's own timeout, measured at nine hours. Giving up is now stated plainly on the job and in the log, and `cancel_run_when_exhausted` is no longer indiscriminate: it cancels only when nothing else in the run is working, and holds back when GitHub cannot be asked. Still off by default, because it remains a whole-run operation. |
